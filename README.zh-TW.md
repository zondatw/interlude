...
用 [`pipx`](https://pipx.pypa.io/)（建議）或
[`uv tool`](https://docs.astral.sh/uv/concepts/tools/) 一次裝好——`interlude`
指令會落在 PATH 上、跑在自己的 venv，不需要任何 project-level 設定：
...
需要 Python 3.11+ 以及你要攔截的 agent CLI（`claude` 跟 / 或 `codex`）。
零 runtime dependency——interlude 只用 stdlib。

要 hack interlude 本身，看下方「[開發環境](#開發環境)」。

## 快速上手

```bash
# 1. 一個指令搞定:proxy 3 個 listener + web UI on :8000
interlude

# 2. 另開終端，把 Claude Code 指過來
ANTHROPIC_BASE_URL=http://localhost:8788 claude

# 3. 開瀏覽器看 live capture
...
```bash
interlude-analyze                    # 讀 .interlude 全部 log
interlude-analyze --agent claude     # 只看單一 agent
interlude-analyze --max-slots 30     # 多印幾條動態插槽
interlude-analyze path/to/log.jsonl  # 指定檔 / glob
```
...
```bash
interlude-report serve                  # http://127.0.0.1:8000（預設）
...
編 `src/interlude/proxy.py` 頂端的 `LISTENERS`，加一行 `(port, upstream_host, label)`。
wire 偵測在 `detect_wire()`、欄位正規化在 `extract()`（請求）與 `reconstruct()`（回應）。

## 開發環境

要 hack interlude 本身，clone repo 直接 src layout 跑：

```bash
git clone https://github.com/zondatw/interlude.git
cd interlude
uv sync                              # editable install
uv run interlude                     # 從 src/interlude/ 跑
```

Contributor：裝 [`pre-commit`](https://pre-commit.com/) 跟
[`gitleaks`](https://github.com/gitleaks/gitleaks)（`brew install pre-commit gitleaks`），
跑一次 `pre-commit install`。之後 `git commit` 會自動跑 ruff lint+format、衛生檢查、
codespell、gitleaks。

Release flow（PyPI Trusted Publishing 走 `beta` / `release` 兩條 branch）寫在
...
| 路徑 | 用途 |
|---|---|
| `src/interlude/proxy.py` | 三 listener reverse proxy，streaming relay + SSE tee/重組。Entry point: `interlude` |
| `src/interlude/analyze.py` | 跨請求 diff、固定骨幹 vs 動態插槽、跨 agent 比較（純文字報告）。Entry point: `interlude-analyze` |
| `src/interlude/report.py` | 本地 web UI（HTML + JSON），對應同樣的分析。Entry point: `interlude-report` |
| `dogfood.sh` | 一鍵端到端驗證（contributor-facing，不會 ship 進 wheel） |
| `docs/release.md` | PyPI Trusted Publishing 設定 + 每次 release 流程 |
| `.github/workflows/` | `beta.yml`（push `beta` → test.pypi.org）、`release.yml`（push `release` → pypi.org） |
| `.interlude/` | JSONL 輸出，落在使用者的 cwd（gitignored，敏感） |
